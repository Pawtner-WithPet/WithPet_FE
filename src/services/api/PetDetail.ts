...
const api = axios.create({
  baseURL: 'http://192.168.209.80:8080',
  timeout: 5000,
  headers: {
...
// 반려견 리스트 조회
export const fetchDogs = async (userId: number) => {
  const res = await api.get('/api/pet/list', { params: { userId } });
...
// 반려견 등록번호로 petId 조회
export const getPetIdByRegNo = async (dogRegNo: string) => {
  const res = await api.get(`/api/pet/regNo/${dogRegNo}`);
  return res.data;
};
